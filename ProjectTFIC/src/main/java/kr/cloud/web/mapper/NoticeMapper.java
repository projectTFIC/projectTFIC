...
public interface NoticeMapper extends JpaRepository<PpeRecord, Integer> {
    ...
    @Query(value =
        "SELECT ti.type_record, COUNT(*) AS cnt " +
        "FROM ppe_record pr " +
    ...
    // 일별, 유형별 집계
    @Query(value =
        "SELECT DATE(ti.reg_date), ti.type_record, COUNT(*) AS cnt " +
        "FROM ppe_record pr " +
        "JOIN type_info ti ON pr.type_id = ti.type_id " +
        "WHERE ti.reg_date BETWEEN :start AND :end " +
        ...
        "SELECT DATE(ti.reg_date), ti.type_record, COUNT(*) " +
        "FROM acc_record ar " +
        "JOIN type_info ti ON ar.type_id = ti.type_id " +
        "WHERE ti.reg_date BETWEEN :start AND :end " +
        "GROUP BY DATE(ti.reg_date), ti.type_record " +
    ...
    // 구역별, 일별, 유형별 집계
    @Query(value =
        "SELECT ti.location, DATE(ti.reg_date), ti.type_record, COUNT(*) AS cnt " +
        "FROM ppe_record pr " +
        "JOIN type_info ti ON pr.type_id = ti.type_id " +
        "WHERE ti.reg_date BETWEEN :start AND :end " +
        ...
        "SELECT ti.location, DATE(ti.reg_date), ti.type_record, COUNT(*) " +
        "FROM acc_record ar " +
        "JOIN type_info ti ON ar.type_id = ti.type_id " +
        "WHERE ti.reg_date BETWEEN :start AND :end " +
        "GROUP BY ti.location, DATE(ti.reg_date), ti.type_record " +
