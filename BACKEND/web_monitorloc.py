...
MIN_INTERVAL = float(os.getenv("AI_MIN_INTERVAL", "0.1"))

# 서버가 권장하는 미리보기 FPS (프론트가 받으면 간격을 맞추게 됨)
SERVER_TARGET_FPS = float(os.getenv("SERVER_TARGET_FPS", "10.0"))
...
try:
    ...
    # 중장비 탐지 모델
    he_model = YOLO("models/last.pt").to(device)          # 최종적으로 he_model.pt 로 이름 수정
        # 중장비 탐지 모델
...
# =================================================================
SPRING_BOOT_API_URL = os.getenv("SPRING_BOOT_API_URL", "http://localhost:8089/web/aiapi")
...
NCP_ACCESS_KEY = os.getenv("NCP_ACCESS_KEY", "ncp_iam_BPASKR5sVNmUgjWifJcD")      
NCP_SECRET_KEY = os.getenv("NCP_SECRET_KEY", "ncp_iam_BPKSKR5HbjmxW3Q7lK8D23L15PuP6C32FR")      
